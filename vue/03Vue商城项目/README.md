...
## 小球动画优化思路
1. 分析导致 动画 不准确的 本质原因，把小球的最终 位移到的 位置，已经局限在某一分辨率下，滚动条未滚动情况下
2. 只要分辨率 和 测试时候不一样，或者滚动条有一定距离后，问题即出现
3. 因此，我们经过分析，得到结论；不能把 位置的 横纵坐标 直接写死了，而是应该 根据不同情况，动态计算这个坐标值
4.  得到起始位置坐标  结束位置坐标，计算好横纵的距离 ，然后设置位置
5.  如何获取 购物车 和 小球位置，domObject.getBoundingClientRect()

## 如何实现加入购物车时候，拿到 选择的数量
1. 分析发现：按钮 属于goodsinfo页面，数字 属于 numbox 组件
2. 由于涉及到了父子组件的嵌套，所以无法直接在 goodsinfo中 获取 选中商品的数量值
3. 涉及到 子组件 向 父组件 传值（事件调用机制）
4. 事件调用的本质：父组件向子组件传递方法，子调用这个方法，同时把 数据 作为传输 传递给此方法
