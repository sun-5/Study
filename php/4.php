...
/*
静态变量
static 是在函数内部定义的变量，使用static 关键字修饰，用来实现【跨函数共享数据】的变量：
函数运行结束所有局部变量都会清空，如果重新运行一下函数，所有的局部变量又会重新初始化

基本语法：
function 函数名称(){
    Static $变量名 = 值；//通常会在定义时候就直接赋值
...
aaa();// 1    1  非静态变量，每次调用，都是重新赋值为1
aaa();// 1    2   静态变量 多次调用，防止初始化
aaa();// 1    3
//静态变量作用为了跨函数共享数据（同一个函数被多次调用）
/*静态变量的使用：
...
//将用户定义的函数传入 给 另一个函数使用，叫做回调函数

/*
匿名函数
没有名字的函数
$变量= function(){} 赋值给变量来调用

闭包
函数内部有一些局部变量，要执行的代码块在函数执行之后没有被释放，
是因为在函数内部还有对应的函数在引用
...
/*
伪类型
实际上在php中不存在的类型，但是通过伪类型可以帮助程序员去更好的查看操作手册，从而更方便学习

Mixed：混合 可以是多种php中的数据类型
Number：数值 可以是任意数值类型（整型 浮点型）
*/
?>